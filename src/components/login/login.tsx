		...
		<section id="login-container" className="login-container">
			...
			<div className="right-login-section">
				<div className="login-window">
					<h1>Login</h1>
					<p>Welcome! Introduce your email and password to sign in into your account.</p>
					<h5>Email</h5>
					<input className="login-input" type="text" value={authorizationRequest.email} onChange={(e) => { updateEmail(e.target.value); }}/>
					<h5>Password</h5>
					<input className="login-input" type="password" value={authorizationRequest.password} onChange={(e) => { updatePassword(e.target.value); }}/>
					<span className="login-error-message" hidden={!errorMessageVisible}>Email or password not correct.</span>

					<button className="login-button" disabled={!authorizationAttemptEnabled} onClick={attemptAuthorization}>Login</button>
